...
export function RewriteScreen({ onBack, appState }: { onBack: () => void; appState?: any; actions?: any }) {
  return (
    <Screen>
      <Pressable onPress={onBack} style={styles.back}><Text style={styles.backText}>← Back</Text></Pressable>
      <Text style={styles.title}>Rewrite My Sentence</Text>
      <Text style={styles.sub}>Steady MVP screen scaffold.</Text>
      <Card>
        <Text style={styles.cardTitle}>What belongs here</Text>
        <Text style={styles.body}>Turn hard-to-start phrases into easier, more speakable versions.</Text>
      </Card>
    </Screen>
  );
}
